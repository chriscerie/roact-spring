...
return function()
    ...
    describe("Controller", function()
        ...
        it("can animate a color string hex", function()
            local styles, api = RoactSpring.Controller.new({ x = "#000" })
            expect(styles.x:getValue().R).to.equal(0)
            expect(styles.x:getValue().G).to.equal(0)
            expect(styles.x:getValue().B).to.equal(0)
            api:start({ x = "#fff" }):await()
            expect(styles.x:getValue().R).to.equal(1)
            expect(styles.x:getValue().G).to.equal(1)
            expect(styles.x:getValue().B).to.equal(1)
        end)
